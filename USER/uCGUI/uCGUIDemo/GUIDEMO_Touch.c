...
/*
----------------------------------------------------------------------
Addition		: UART Integration
Created By		: Siddharth Kaul and Nandan Mehta
...
void init_uart0(void)
{
	SystemInit();
	UART0_Init();
	UART0_SendString("UART0 is initialised\n");
...
void send_press_key(void)
{
	int temp_key;
	//unsigned char temp_x_char;
	do
	{
		temp_key = _ExecKeyboard();
		//temp_x_char = _acText[temp_key];
		//UART0_SendString(_acText[temp_key]); 
	} while(temp_key!=GUI_ID_CANCEL);
}
#else
...
**********************************************************************
*/
/* Will be directly executing the keyboard */
#if GUIDEMO_LARGE
